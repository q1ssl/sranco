...
frappe.ui.form.on("Representative Commission Statement", {
...
frappe.ui.form.on("Representative Commission Statement Table", {
...
function get_sales_invoice_data(frm) {
    if (frm.doc.from && frm.doc.to && frm.doc.representative) {
        console.log("get_sales_invoice_data");
        frappe.call({
            method: "sranco.sales_invoice.get_rep_sales_invoice_list",
            args: {
                from_date: frm.doc.from,
                to_date: frm.doc.to,
                representative: frm.doc.representative,
            },
            callback: function (r) {
                if (r.message) {
